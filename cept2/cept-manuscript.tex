...
%
Derivation of the working equations of our PNO-CEPT is based on the reduced spin-free excitation operators
%
\begin{align}
  &E^w_x = \sum_{\sigma=\{\alpha,\beta\}} a^{w\sigma}a_{x\sigma} \\
  &E^{wx}_{yz} = \sum_{\sigma,\tau=\{\alpha,\beta\}} a^{w\sigma}a^{x\tau}a_{z\tau}a_{y\sigma}
\end{align}
%
...
\end{align}
%
where $f$ and $E$ represent the generalized Fock matrix and the spin-free excitation operator, respectively.
%
The generalilized Fock matrix is defined as



\subsection{The Coupled-Electron Pair Perturbation Theory}\label{Subsec:cept2}
%
The CCSD wave function is defined with an exponential operator acting on the reference HF function ($\Psi_0$)
%
