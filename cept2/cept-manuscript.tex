...
In this paper, we report a hybrid of IC-MR-CEPA and CASPT2, which is conined CEPT2, as a higher-order, yet simple extenton to the conventional CASPT2 method.
%
The 2-external configurations in the first-order CEPT wave function are treated at the IC-MR-CEPA level while the remaining semi-internal and the internal configurations are kept at the CASPT2 level for capturing more dynamic correlation.
%
...
%
In the CIPT2 extension to the CASPT2 theory, the reference configurations, ({\it active}, {\it active})$\rightarrow$({\it active}, {\it virtual}) and ({\it active}, {\it active})$\rightarrow$({\it virtual}, {\it virtual}) excited configurations are treated at the MRCI level while leaving the remaining ones at the CASPT2 level in order to remedy the intruder state problem without employing the real\cite{ROOS1995215} and imaginary\cite{FORSBERG1997196} shifts.
%
%Even though their CIPT2 model is based on partially-IC formulation,\cite{Celani2000,PCMRCI2011} the IC configurations involving three active MO indices, which are solved at the MRCI level 
%
Actually, treating the configurations with three active MO indices in the fully IC-MRCI level framework is know to cause several difficulties:
%


one of the semi-internal IC configurations with three active MO indices are treated at the MRCI level thus requiring evaluation of tensor contractions involving the four-body RDM (4-RDM) and the amplitudes 

%
Therefore, in the closed-shell single-reference limit, the CEPT2 model converges to the linearized coupled-cluster doubles (L-CCD) or equivalently the CEPA/0 doubles (CEPA/0(D)) that recovers more dynamic correlation energy than the MP2 model.
%
...
%
On the other hand, the CASPT2 and NEVPT2 models converge to the MP2 in the closed-shell single-reference limit.
